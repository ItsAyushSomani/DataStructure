...
public class Q1LCS {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Length of LCS memo is" + " " +
                lcsRecursion(X, Y, n, m, dp));
//        System.out.println();
//        //Memoization
...
//                lcsTopDown(X, Y, n, m));
    ...
    public static int lcsTabu(char[] arr, char[] brr, int n, int m) {
        int[][] dp = new int[n + 1][m + 1];
        for(int i = 1; i <= n; i++) {
            for(int j = 1; j <= m; j++) {
                if(arr[i - 1] == brr[j - 1]) {
    ...
    public static int lcsRecursion(char[] arr, char[] brr, int n, int m, int[][] dp) {
        ...
        if(arr[n - 1] == brr[m - 1]) {
            ...
            return dp[n][m] = 1 + lcsRecursion(arr, brr, n - 1, m-1, dp);
        }
        return dp[n][m] = Math.max(lcsRecursion(arr, brr, n - 1, m, dp) , lcsRecursion(arr, brr, n, m - 1, dp));
    }
